...
public class GraphOfTheGodsFactory {
	 ...
	 * file path which must be the sole element in the {@code args} array, then
	 * calls {@link #load(com.thinkaurelius.titan.core.TitanGraph)} on the
	 * opened graph, then calls
	 * {@link com.thinkaurelius.titan.core.TitanGraph#close()} and returns.
	 * <p/>
	 * This method may call {@link System#exit(int)} if it encounters an error,
	 * such as failure to parse its arguments. Only use this method when
	 * executing main from a command line. Use one of the other methods on this
	 * class ({@link #create(String)} or
	 * {@link #load(com.thinkaurelius.titan.core.TitanGraph)}) when calling from
