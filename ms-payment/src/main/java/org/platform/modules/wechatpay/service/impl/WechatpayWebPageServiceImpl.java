...
import org.platform.modules.wechatpay.utils.WechatpayContants;
...
public class WechatpayWebPageServiceImpl implements IWechatpayService {
	...
	public Map<String, Object> mreadPaymentRequest(String identity) throws BusinessException {
		...
		params.put("appid", WechatpayContants.APP_ID);
		params.put("mch_id", WechatpayContants.MCH_ID);
		params.put("body", WechatpayContants.BODY);
		params.put("trade_type", WechatpayContants.TRADE_TYPE);
		params.put("notify_url", WechatpayContants.NOTIFY_URL);
		...
		params.put("sign", WechatpayUtils.genSignature(params));
		...
		sparams.put("appid", WechatpayContants.APP_ID);
		sparams.put("partnerid", WechatpayContants.MCH_ID);
		sparams.put("prepayid", resultMap.get("prepay_id"));
		sparams.put("package", WechatpayContants.PACKAGE);
		sparams.put("noncestr", resultMap.get("nonce_str"));
		sparams.put("timestamp", WechatpayUtils.genParamTimeV2());
		String secondSign = WechatpayUtils.genSignature(sparams);
	...
	public String verifyPaymentNotifyCallback(Object callbackParams) throws BusinessException {
		...
		if (!WechatpayContants.MCH_ID.equals(notifyParams.get("mch_id"))) {
		...
		String vsign = WechatpayUtils.genSignature(vparams);
	...
	private void insertRechargeRecord(RechargeMode rechargeMode, String outTradeNo, Map<String, Object> params) {
		...
		rechargeRecord.setChannel(WechatpayContants.CHANNEL);
		rechargeRecord.setOutTradeNo(outTradeNo);
